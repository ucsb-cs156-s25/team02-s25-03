...
describe("ArticlesTable tests", () => {
  ...
  test("Edit button navigates to the edit page", async () => {
    ...
    ).toHaveTextContent(String(articlesFixtures.threeArticles[0].id));
    expect(
      screen.getByTestId(`${testId}-cell-row-0-col-title`),
    ).toHaveTextContent(articlesFixtures.threeArticles[0].title);
    ...
    await waitFor(() =>
      expect(mockedNavigate).toHaveBeenCalledWith(
        "/articles/edit/" + String(articlesFixtures.threeArticles[0].id),
      ),
    );
  });
  ...
  test("Delete button calls delete callback", async () => {
    ...
    expect(axiosMock.history.delete[0].params).toEqual({
      id: articlesFixtures.threeArticles[0].id,
    });
  });
});
